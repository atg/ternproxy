...
(function(mod) {
  ...
  function isSimpleAnnotation(spec) {
    ...
    for (var prop in spec)
      if (prop != "!html" && prop != "!type" && prop != "!doc" && prop != "!url" && prop != "!span")
        return false;
    return true;
  }
  ...
  function passTwo(base, spec, path) {
    ...
    for (var name in spec) if (hop(spec, name) && name.charCodeAt(0) != 33) {
      ...
      if (typeof inner == "string") {
        ...
        var doc = inner["!doc"], url = inner["!url"], span = inner["!span"], html = inner["!html"];
        if (doc) {
        ...
        if (html) {
          if (type && type instanceof infer.Obj) type.html = html;
          known.html = html;
        }
      }
    }
